...
export default function DeleteCategory({
  ...
  async function handleDelete(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/categories/${selectedData.id}`, {
        method: "DELETE",
        headers: {
      ...
      console.log("DELETE CATEGORY STATUS:", response.status);
      console.log("DELETE CATEGORY RESULT:", result);

      if (!response.ok) {
        alert(getErrorMessage(result, "Gagal menghapus kategori"));
        return;
      }
  ...
  return (
    <AlertDialog open={open} onOpenChange={setOpen}>
      ...
      <AlertDialogContent
        className="w-[92vw] overflow-hidden p-0 sm:max-w-md"
        style={{
          borderRadius: 24,
        ...
        <form onSubmit={handleDelete}>
          ...
          >
            ...
            <AlertDialogHeader className="relative z-10 space-y-0 text-left">
              ...
              >
                ...
                akan dihapus. Pastikan kategori ini tidak sedang dipakai di
                catering plan.
              </AlertDialogDescription>
            </AlertDialogHeader>
          </div>
          ...
          <div className="px-5 py-5 sm:px-7">
            ...
            >
              ...
              <p className="mt-1 break-words text-sm font-bold text-[#1e2a04]">
          ...
          {/* FOOTER */}
          <AlertDialogFooter
            className="flex flex-col-reverse gap-2 px-5 py-5 sm:flex-row sm:justify-end sm:px-7"
