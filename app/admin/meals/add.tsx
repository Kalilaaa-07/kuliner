...
import { ChevronDown, ImagePlus, Plus, Save, Upload } from "lucide-react";
...
export default function AddMeal({ onSuccess }: AddMealProps) {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      console.log("CREATE MEAL BODY:", body);
      ...
      console.log("CREATE MEAL RESULT:", result);

      if (!response.ok) {
        alert(getErrorMessage(result, "Gagal menambah meal"));
        return;
      }

      alert(result?.message || "Meal berhasil ditambahkan");
      setOpen(false);
      resetForm();
      onSuccess();
    } catch (error) {
      console.error("CREATE MEAL ERROR:", error);
      alert("Terjadi kesalahan saat menambah meal");
    } finally {
      setLoading(false);
    }
  }
              ...
              <div>
                ...
                {plans.length === 0 && !plansLoading && (
                  <p className="mt-2 text-xs font-semibold text-red-500">
                    Belum ada catering plan. Buat catering plan dulu sebelum
                    menambah meal.
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                >
                  <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                    ...
                    >
                      <Upload size={16} />
                      {uploadLoading ? "Uploading..." : "Pilih dari Galeri"}
                    ...
                    <p className="text-xs leading-5 text-[#6B705C]">
                      JPG, PNG, atau WEBP. Setelah upload berhasil, preview
                      gambar muncul di bawah.
                    </p>
                  </div>
