...
export default function EditPlan({
  ...
  function openModal() {
    ...
    setPrice(String(selectedData.price || ""));
    setDuration(String(selectedData.duration || ""));
    setCategoryId(
      String(selectedData.categoryId || selectedData.category?.id || "")
    );
    setIsActive(selectedData.isActive === true);
    setOpen(true);
  }
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      alert(result?.message || "Catering plan berhasil diedit");
      setOpen(false);
      onSuccess();
    } catch (error) {
      console.error("UPDATE PLAN ERROR:", error);
      alert("Terjadi kesalahan saat mengedit catering plan");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent
          className="max-h-[90vh] w-[92vw] overflow-y-auto p-0 sm:max-w-lg"
          style={{
            borderRadius: 24,
          ...
          <form onSubmit={handleSubmit}>
            ...
            >
              ...
              <button
                type="submit"
                disabled={loading}
                className="flex items-center justify-center gap-2 rounded-xl px-6 py-2.5 text-sm font-semibold text-white transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:translate-y-0 disabled:opacity-50"
                style={{
                  background: loading
                    ? "#a8c255"
